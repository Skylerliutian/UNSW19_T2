...
def forward_msg(graph,addr):
    ...
    forward_port = []
    its_port = [i for i in graph[forward_who]]
    for i in neighbor_port:
        if i not in its_port:
            forward_port.append(i)
    send_msg(forward_port, graph)
...
def recv_msg():
    while True:
        ...
        # update_graph(graph)
        forward_who = list(graph.keys())[0]
        if forward_who in forward_table:
            if forward_table[forward_who] == addr:
                forward_msg(graph)
                update_graph(graph)
        else:
            forward_table[forward_who] = addr
            forward_msg(graph)
            update_graph(graph)
...
def update_graph(graph):
    ...
    # if recv_id not in updating_graph:
...
if __name__ == "__main__":
    ...
    # global variable to record forward information
    forward_table = {}
